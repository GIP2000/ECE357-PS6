...
void childHandler(struct sem* shells,pid_t my_pid,int task1,int task2,int rockCount){ 
...
int main(int argc, char* argv[]){
    ...
    if (argc == 1){
        ...
        if(rockCount <= 0){
            fprintf(stderr,"Error: rockCount is below 1\n Usage: ./P2C [rockCount] \n");
            exit(1); 
        }
    } else {
    ...
    if(ftest == 0){ // child
        childHandler(shells,my_pid,task1,task2,rockCount);
        return 0; 
    }
    // parent
